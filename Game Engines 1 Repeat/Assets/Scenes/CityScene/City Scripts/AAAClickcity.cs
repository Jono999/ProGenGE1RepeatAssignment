...
public class AAAClickcity : MonoBehaviour
{
    ...
    {
        for (int i = 1; i < HousePoints.Count - 1; i++)
        {
     ...
     public void ControlPoints(List<Vector3>TheFirstList)
    {
        ...
        // below is not functional for what i want// no longer true
           ...
           // for (int j = 0; j < TheFirstList.Count - 1; j++)
            //{
            int randomRightIndex = Random.Range(0, AllPoints.Count - 1);// these and how they're used below is what im talking about above allpoints
